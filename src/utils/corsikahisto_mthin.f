...
C        of file to read (with name DATnnnnnn-xxxxxx from parallel run)
C     2) Execute this program with the file 'input' as standard input:
C              ./corsikahisto_mthin <input >output
...
C=======================================================================
     ...
     & ,specr(maxin,maxie,maxjz,maxjr),speca(maxin,maxjz,maxjr)
      parameter (nweimx=120)
      double precision cntwei(0:nweimx,maxiz),c2w
      common/countweight/cntwei,c2w
...
c                 if(ii.eq.4.and.wt.gt.1d0)print *,'weight',id,wt !<-----------
                 if(pass.and.wt.gt.0)
     .           call cana(h1,x1,y1,t1,E1,px,py,pz,am,wt,gen,idi,iimode)
               enddo
             else
...
c        added----------vc300304---from cana
      ...
      iz=iimode
      iwei=min(nweimx,1+int(log10(max(1d0,wt*c2w))*10d0))
      cntwei(0,iz)=cntwei(0,iz)+1d0
      cntwei(iwei,iz)=cntwei(iwei,iz)+1d0
...
c end column names
      ...
      if(numiz.gt.1)then
      write(ifho,'(a)')'!---------------weight------------'
      write(ifho,'(a)')  'zone 2 2 1 openhisto name wei'
      write(ifho,'(a)')  'htyp lin'
      write(ifho,'(a)')  'xmod log ymod lin'
      write(ifho,*)  'xrange ',1.,eprima
      write(ifho,'(a)')  'yrange auto auto '
      write(ifho,'(a,1p,e10.3,a)')  'text 0.1 0.9 "Prim. Engy (eV) ='
     &                           ,eprima*fev,'"'
      write(ifho,'(a)')  '- txt  "xaxis Thinning Weight W"'
      do iz=2,numiz
      write(ifho,'(a,i1,a)')  '+ txt  "yaxis P(W?',iz,'!)"'
      enddo
      write(ifho,'(a,d22.14)')  'histoweight ',dble(nshower)
      write(ifho,*)  'array ',-1-numiz+1
      iwei=min(nweimx,1+int(log10(max(1d0,eprima*c2w))*10d0))
      do iw=1,iwei
        wi=10d0**(dble(iw-1)/10d0)/c2w
        write(ifho,'(e13.5,90e11.3)')wi
     & ,(cntwei(iw,iz)/max(1d0,cntwei(0,iz)),iz=2,numiz)
      end do
      write(ifho,'(a)')       '  endarray'
      write(ifho,'(a)')       'closehisto'
      do iz=1,numiz-1
       if(iz.le.9)write(ifho,'(a,i1,$)')  ' plot wei+',iz
       if(iz.gt.9)write(ifho,'(a,i2,$)')  ' plot wei+',iz
      enddo
      write(ifho,'(a)')' '
