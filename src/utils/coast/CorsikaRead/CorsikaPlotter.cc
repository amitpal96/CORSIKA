 ...
 */
    ...
    while (cr.GetShower(Shower)) {
      ...
      for (int iObsLevel=1; iObsLevel<=nObsLevel; ++iObsLevel) { 
        ...
        emptyLevel.data = new TNtupleD(tName.str().c_str(), tTitle.str().c_str(), "id:ek:px:py:pz:x:y:t:w");
...
double 	GetE () const 
		  ...
		  if (obsLevel.count(level)==0) {
		    ...
		    obsLevel[level].data = new TNtupleD(tName.str().c_str(), 
							tTitle.str().c_str(),
							"id:ek:px:py:pz:x:y:t:w");
		  }
		  
                  obsLevel[level].data->Fill(id,ek,px,py,pz,x,y,t,w);
