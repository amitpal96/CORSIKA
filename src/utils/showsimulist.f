...
c  ---------------------------
c     creates tabular of air shower simulations done with corsika
c     by reading only the first record of all particle data files
c     [DAT,CER]iiiiii[.cher,.part] and possibly additional reading of
c     the corresponding protocol file as 'DATiiiiii.lst'; otherwise
c     additional conditions must be added to the code; a tabular of one
c     line per file will be created with the following quantities: 
c Primary, lg(E), theta, phi, nsh,  runnr, size, obslvme, h1stme,
c thilev, thiwmax, lg(thirad), verspgm, models, rundate, Xmagn, Zmagn;
c     Cherenkov data will be identified by '_ce' after the primary code.
c ----------------------------------------------------------------------
c CompLink:
c     gfortran -O0 -fbounds-check showsimulist.f -o showsimulist
c     ifort -C -O0 -check bounds showsimulist.f -o showsimulist 
c RunProg:
c     ls -l DAT?????? | ./showsimulist > showsimulist.ca2-showtest
c     ls -l DAT??????.part | ./showsimulist
c     ls -l DAT?????? DAT??????.part | ./showsimulist
c     ls -l DAT?????? DAT??????.???? | grep -v l | ./showsimulist
c     ls -l DAT??????.cher | ./showsimulist
c     ls -l CER?????? | ./showsimulist > showsimulist.ca2-cherenkov
c using file names in text file:
c     ./showsimulist < showsimulist.datnames
c using 'simprod' simulations:
c     ls -l [c,f,p]?e??m???_* | ./showsimulist
c-----------------------------------------------------------------------
...
c--define some more constant and quantities-----------------------------
      qpatext(176) = ' Stnd              '
      qpatext(196) = ' Thin              '
      cpi = 4.d0 * atan(1.d0)
      cpi180 = cpi/180.d0
...
c--read first file name and fix position of file size-------------------
  ...
  401 continue
     ...
     +      '' with `-rw`; first use `chmod u+w *`;'')')
      ...
      read(cdat(itask+3:itask+8),'(i6)') irunnr
      ntskf(ifil) = irunnr
...
c-----------------------------------------------------------------------
c--loop over all particle data files------------------------------------
      do  444  ifil=1,nfil
...
c - - - - - check on 64 or 32 bit and standard or thinning corsika:
     ...
     +   isubr=273
      ...
      enddo
      do  i=1,isubr
         qevth(i) = dble(pdata(ibit+isubr+i))
      enddo
      irunnr = ntskf(ifil)
...
c - - - - - check models and date of simulation - - - - -
      ...
      enddo
      imont = int(mod(qevth(45),1.d4))
      ijahr = int(qevth(45)*1.00001d-4)
...
c - - - - - check on former 64bit simulation and Cherenkov output - - -
...
c - - - - - check on thinning parameters - - - - -
      if ( qevth(151) .lt. 1. ) then
         isla = int(log10(qevth(4))+log10(qevth(148)+4.67735141e-34))
         islb = 10 
         qevth(151) = 0.
         do  i=1,isla
      ...
      ! qevth(150) = Wmax_hadronic. qevth(151) = Wmax_elmagnetic.
...
c - - - - - print out tabular of shower quantities - - - - -
      if ( ip .gt. 0 ) then
         if ( int(qevth(44)) .ne. irunnr ) qevth(44) = irunnr
         mcode = int(qevth(3)) ! mcode=0 for stackin.
  ...
  444 continue 
c--end-of loop over all particle data files-----------------------------
c-----------------------------------------------------------------------
      close(unit=3)
...
c--print closing comment lines------------------------------------------
     ...
     +   2x,''verspgm    models   rundate  Xmagn  Zmagn'',/)')
  445 continue
      write(*,'(14x,''Total number of files:'',i8)') nfilstop
...
c--print explanation of model digits------------------------------------
     ...
     + 1x,''(10^7): EGS flag; (10^6): NKG flag; (10^5):'',
