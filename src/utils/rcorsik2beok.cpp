...
//    i.e. closing EVTE and RUNE subblocks.
// - - - - - - - - - - - - - - CompLinkRun - - - - - - - - - - - - - - - -
// CompLink:
//    g++ -O0 -fbounds-check rcorsik2beok.cpp -o rcorsik2beok -lm
// RunProgr:
//    ./rcorsik2beok < rcorsik2beok.name
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// input-file 'rcorsik2beok.name' (file names without quotes):
//    DAT000333 
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
// = = = = = = = = = = = = = = global constants  = = = = = = = = = = = = =
   ...
   const int nmaxrec = 2200000;              // valid up to 50 GBytes.
...
//=== II. - read file names - - - - -
   for( ifil=1; ifil<50000; ifil++ ) {
      if ( cin.peek() == EOF ) {
         ifil--;
         cin.clear();
         break;
      }
      cin.getline(chtext,240);
      itext = strlen(chtext);
      chtext[itext+1] = '\0';
