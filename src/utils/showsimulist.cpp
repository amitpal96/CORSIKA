...
char corsfile[nfmx][250];

int nfsho[nfmx], nfrun[nfmx], nflen[nfmx], nbits[nfmx];
...
int main(int argcnt, char *argvec[])
{
   ...
   int ifil, idate, itext, iprmtxt, nfilstop, nplotsh, mcodprev, ibit,
       isla, lthi, im, lthiprev, irunnr, imont, ijahr, nfil, nsblock, 
       itaskcer, itaskchv, mcode, models, mjahr, mmon, mtag, mhour, mmint;

   char chlsline[256], chtext1[14][256], chrunnr[16], cprimary[16],  
        chartest[4][8]={" ","CER","DAT","cher"},
...
// ------------------------------------------------------
   ...
   mjahr = 2017;
   if ( strcmp(chtext1[8],":") != 0 ) {
      mjahr = atoi(chtext1[8]);
   }
   else {
...
// -----------------------------------------------------------------------
   ...
   for( ifil=1; ifil<nfil; ifil++ ) {
      ...
      lthi = -1;
      nbits[ifil] = ibit;
      for( im=0; im<numbthi; im++ ) pdata[im] = -1.;
      itaskcer = mposubstring( corsfile[ifil], chartest[1]); // "CER"
      itaskchv = mposubstring( corsfile[ifil], chartest[3]); // "cher"
      nfilstop++;
   // - - - - - - - - - - - - - - - - - - - - - - -
      ...
      if ( inputcors.peek() == EOF ) {
         ...
         if ( fabs(pdata[1]) < 1.e-6 ) {
            if ( 211285.2 < pdata[2] && pdata[2] < 211285.3 ) ibit = 64;
            // found first two floats as zero (former 64 bit simul.)
            nbits[ifil] = ibit;
         }
         ibit = ibit / 64;
         lthi = -1;
         if (217433.0 < pdata[nsblstd+1+ibit] &&
                        pdata[nsblstd+1+ibit] < 217433.1) {
            lthi = 0;
         }
         if (217433.0 < pdata[nsblthi+1+ibit] &&
                        pdata[nsblthi+1+ibit] < 217433.1) {
            lthi = 1;
         }
         nsblock = nsblstd + lthi*39; // ndatpar = nsblock / 39;
         for( im=0; im<=nsblock; im++ )
              qrunh[im] = pdata[im+ibit];
         for( im=nsblock+1; im<=nsblock+nsblstd; im++ )
              qevth[im-nsblock] = pdata[im+ibit];
         // checked qrunh[0] and qrunh[1] = 211285.2812500000000000;
      }
   // - - - - - - - - - - - - - - - - - - - - - - -
   // - - - - - check on primary particle - - - - -
      if ( 0 < qevth[3] && qevth[3] < 200. ) {
         ...
         // print out should not - but may - occur.
      }
   // - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   // - - - - - check models and date of simulation - - - - -
      ...
      strcpy(&cprimary[11], "\0");
   // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   // - - - - - check on former 64bit and/or Cherenkov simulation - - - -
      if ( nbits[ifil] == 64 ) {
         // should not occur, except as former test case:
         if ( itaskcer > 0 ) {
            strcpy(&cprimary[8], "_c4");
            strcpy(&cprimary[11], "\0");
         }
         else {
            strcpy(&cprimary[8], "_64");
            strcpy(&cprimary[11], "\0");
         }
      }
      else {
         if ( itaskcer > 0 || itaskchv > 0 ) {
            strcpy(&cprimary[8], "_ce");
            strcpy(&cprimary[11], "\0");
         }
      }
   // - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   // - - - - check previous parameters of shower - - - - -
      if ( iprmtxt > 0 ) {
        mcode = int(qevth[3]);
        if ( mcode == 0 ) mcode = 4; // stackin simul.
        energy = qevth[4];
        ...
        if ( mcode != mcodprev || energy != engyprev ||
           ...
           // if ( ifil > 1 ) printf("  \n"); // check extra blank line.
        }
   // - - - - - - - - - - - - - - - - - - - - - - -
   // - - - - print out tabular quantities- - - - -
        if ( fsize[ifil] < 10000. ) {
          printf("%s%5d  %6.2f %5.1f %7.2f %4d  %.6d"
          " %7.1f %8.2f %7.0f. %5.1f %8.1e %5.1f %8.5f  %.8d"
          ...
          printf("%s%5d  %6.2f %5.1f %7.2f %4d  %.6d"
          " %6.0f. %8.2f %7.0f. %5.1f %8.1e %5.1f %8.5f  %.8d"
          " %9d %6.2f %6.2f\n", cprimary, int(qevth[3]), energy,
          c180pi*qevth[11], c180pi*qevth[12], nfsho[ifil],
          int(qevth[44]), fsize[ifil], 1.e-2*qevth[48],
          1.e-2*qevth[7], log10(qevth[148]+4.67735141e-34),
...
// --end-of loop over all particle data files-----------------------------
// -----------------------------------------------------------------------
