...
#          -bug fix: correspondence between MPI Send/Receive data types         #
#          -bug fix: proper offsets calculation for MPI Struct type             #
#          -GH fit calculation                                                  #
#    v0.61 xx-09-19   Tim Huege                                                 #
#          -bug fix: proper handling of paths within quotes                     #
#    v0.62 17-10-19   Tim Huege                                                 #
#          -detect if simulation does not use all cores, print out warning      #
#           message and stop for CoREAS simulations or efficiencies <85%        #
#    v0.63 30-06-20   Tim Huege                                                 #
#          -detect if particle-stack array is overrun and abort with error      #
#                                                                               #
#    ToDo:                                                                      #
...
int run_inefficiently=0; //switch to override the stopping of inefficient simulations
...
struct book   //Keeps the information about whether a processor is "Busy(1)" or "Free(0)"
{
	int busy;
	int id; 
  int exhausted; // if 1, slave will not request any additional new particles to be worked on during its current work package
  int initialized; // if 1, slave has received a work package in the past and is thus initialized
};
...
/*
function check_for_all_running_exhausted()
Called from main() by the MASTER to check if all busy slaves are exhausted
Returns TRUE if this is the case, false otherwise
*/
int check_for_all_running_exhausted(struct book* book,int num_process)
{
 int i,check=1;
 for(i=1;i<num_process;i++)
 {
 if((book[i].busy==1) && (book[i].exhausted==0))
  {
  check=0;
  break;
  }
...
/*
function get_num_initialized()
Called from main() by the MASTER to retrieve number of initialized slaves
Returns number of initialized slaves
*/
int get_num_initialized(struct book* book,int num_process)
{
 int i,num_initialized=0;
 for(i=1;i<num_process;i++)
 {
 if(book[i].initialized==1)
  num_initialized++;
 }
...
void read_input(double* dectcut,double* dectmax,int* lprim,char* cutfilename,int* id1,int* id2,int* rnnum,int* seed,char * input_fname)
{
 int x,dum;
 char* end;
 char str1[10],str2[180],str3[32],str4[32],str5[2],str[255];
	...
	if(rank==MASTER)
	{
		...
		else
		{
			...
			if(argc>=4) 
				...
				{ 
					run_inefficiently=1; 
					printf("Simulation will not stop even if it is inefficient (using less than 85%% of the cores for actual simulation).\n");
				}
		...
		// Broadcasting the run number to all slaves
		...
		//initialize all ranks as free and not exhausted and not initialized
		for(i=0;i<num_process;i++) {
       book[i].busy=0;
       book[i].exhausted=0;
       book[i].initialized=0;
    }
		...
		//Send the order to start the parallel CORSIKA task on slave 1 with id=buff.uid
...
master_back:; 
      ...
      int source_rank = stat.MPI_SOURCE;
			...
			{
			case EXHAUSTED:
      //Slave sent an 'EXHAUSTED' message, i.e. it will not request new particles from its current work package
      {
        double dummy = 0.;
        MPI_Recv(&dummy,1,MPI_DOUBLE,source_rank,EXHAUSTED,MPI_COMM_WORLD,&stat);

        book[source_rank].exhausted=1;  // mark slave as exhausted
        int num_initialized=get_num_initialized(book,num_process);
        if(check_for_all_running_exhausted(book,num_process) && (num_initialized<num_process-1))
        {
          //all running jobs are exhausted, but there are still uninitialized slaves -> we are running inefficiently
          float efficiency=num_initialized/(float)(num_process-1)*100;
          printf("\n*************************************************************************\n");
          printf("* WARNING: The simulation is not using all cores!\n");
          printf("*          Number of cores used by MPI: %d \n",num_process);
          printf("*          Number of cores usable for simulation: %d \n",num_process-1);
          printf("*          Number of cores used for simulation: %d \n",num_initialized);
          printf("*          Number of cores without work: %d \n",num_process-1-num_initialized);
          printf("*          Efficiency of simulation (exluding bookkeeping): %.1f %% \n",efficiency);
          printf("* Please adjust PARALLEL keyword in input file or use fewer cores.\n");
          printf("*************************************************************************\n");
          if((efficiency<85) && (!run_inefficiently))
          {
            printf("\nERROR: Efficiency is below the threshold of 85%%. Aborting ...\n");
            printf("If you want to run this job anyway, you can override with passing \"T\" as the fourth argument to mpirun.\n");
            MPI_Abort(MPI_COMM_WORLD,mpierr);
          }

#if __COAST__
          //CoREAS simulations will fail at data aggregation if cores never received particles, so have to stop here in any case.
          printf("\nERROR: CoREAS cannot process final data if cores remain uninitialized. Aborting ...\n\n");
          MPI_Abort(MPI_COMM_WORLD,mpierr);
#endif            
        }
 goto master_back;
...
goto master_back;
          ...
          book[child_rank].initialized=1;
          book[child_rank].exhausted=0;
...
goto master_back;
          ...
          book[free_rank].initialized=1;
          book[free_rank].exhausted=0;
...
slave_back:;
		...
		particle_stack_or_cut_file = (double*)calloc((PARTICLE_INFO_COUNT+1)*MXLIST,sizeof(double)); //Stack for possible new secondary particles
                sizeofparticlestack = (PARTICLE_INFO_COUNT+1)*MXLIST; // keep track of size of particle stack
...
void writeparticle_(double* cutpar,int* uid,int* i)
{
 ...
 if ((*i-1)*(PARTICLE_INFO_COUNT+1)+(PARTICLE_INFO_COUNT-1) >= sizeofparticlestack) {
  printf("\nParticle stack is overrun. Please set reasonable values for parallelization such as PARALLEL 1000 (EPRIMARY/GeV/10/#cores) - aborting ...\n\n");
...
/*
function slaveexhausted()
Called from Subroutine CORSIKA() to signal that this slave will not send more particles
It signals this to the MASTER
*/
void slaveexhausted_()
{
  double dummy = 0.;
  MPI_Send(&dummy,1,MPI_DOUBLE,MASTER,EXHAUSTED,MPI_COMM_WORLD);   
}
...
void printstatusstop_(int* rnnum,int* uid,char cftext[255])
{
 ...
 MPI_Comm_rank (MPI_COMM_WORLD, &rank);
	...
	strcat(statfile,"-runprotocol.txt");
	file = fopen(statfile,"a"); 
	fprintf(file,"\n\n\n[%f] CORSIKA: Parallel simulation of run number %d stopped :%s \n\n",
