...
c-----------------------------------------------------------------------
     ...
     +          jseedq(6)
      ...
      data nmuthi/0/, nthi/0/, nsimul/0/, lthi/0/, jseedq/1,2,3,4,5,6/
...
c - - - - - - read all Job files and keep names in a character array - -
  ...
  102 continue
      ...
      nbxmag = 0 ! count coreas simulations.
      mrun = 0
...
c - - - - - - read and check content of each file Job*.out - - - - - -
      ...
      nsimul = nsimul + 1 ! count total numer of simulations.

c - - - - get current number of subshowers and coreas info of Job*.err:
     ...
     +     form='formatted',access='sequential',status='old')
      ...
      ! or read(1,*,end=103,err=103) jbxmag,gbzmag,gbymag
  103 continue
      close(unit=1)
      if ( jbxmag .gt. 0 ) then
         nbxmag = nbxmag + 1
         gbxmag = jbxmag ! found coreas number of Antennas.
         bxmag = gbxmag
         bzmag = gbzmag 
      endif
      tminuts = 0.987654
...
c - - - - implicit loop on lines in file Job*.out:
  ...
  104 continue
      ...
      do  j=1,240
         if ( chzeile(j:j) .eq. char(9) ) chzeile(j:j) = ' '
      enddo
  ...
  119       continue
      ...
      ! - - - magnetic field components - - - - - - - - - - - - - - - -
         ...
         if ( jmag .ge. 1 .and. jbxmag .eq. 0 ) then
            jbl1 = index( chzeile, ' ') 
  120       continue
            jbl1 = jbl1 + 1
  ...
  127       continue
      ...
      ! - - - multi thinning specifications- - - - - - - - - - - - - - -
         ...
         if ( jthm .ge. 1 ) then
            mthinh = mthinh + 1
            if ( mthinh .eq. 1 ) nmuthi = nmuthi + 1
            jbl1 = index( chzeile, ' ')
  128       continue
            jbl1 = jbl1 + 1
...
c - - - - - - time analysis from file time.txt - - - - - old hc3 - - - -
  ...
  148    continue
     ...
     +      = chjobname(jj)(1:10)//'time'//chjobname(jj)(4:9)//'.txt'
         ...
         ! tminuts already available from Job*_scc.out (see above).
      endif
...
c - - - - - - print last title line of tabular - - - - - - - - - - - - -
      if ( nbxmag .gt. 0 ) then ! found coreas: 
        write(*,'(/,''primary  lg(E)  theta   phi   runnr  sizeGBy'',
     +      2x,''procs  T(days)  ecutmax  t(min)  files'',
     +      3x,''RATIO'',2x,''obslev'',2x,''Antens'',3x,''gamma'',
     +      2x,''_corsika_executable_'')')  
     ...
     +      2x,''_corsika_executable_'')')  
      ...
      write(*,'(54x,''Total'',13x,''Wall'',9x,''E/ecutmax'')')
      write(*,'(/,50x,''Total number of simulations:'',i5)') nsimul
      if ( nthi .gt. 0 )
     +   write(*,'(/,50x,''Number of thin simulations :'',i5)') nthi
      if ( nmuthi .gt. 0 )
     +   write(*,'(/,50x,''Number of multithin simuls.:'',i5)') nmuthi
      if ( nbxmag .gt. 0 )
     +   write(*,'(/,50x,''Number of coreas simulation:'',i5)') nbxmag
      write(*,'(2x)')
