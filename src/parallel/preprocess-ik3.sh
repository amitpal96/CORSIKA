...
# -------------------------------------------------------------------------
# usage:
# ./preprocess-ik3.sh parallel-003210 corsika75601_stnd_QGSII4_gheisha_pll
# cd csk003210/
# ./corsika-initial-ik3.sh parallel-003210 corsika75601_stnd_QGSII4_gheisha_pll ik3
# -------------------------------------------------------------------------
...
# - - - - - - switch to subdirectory and optionally create epos path: 
  ...
  cp ../VENUSDAT .
  echo "           ... copying utility codes and scripts;"
  cp ../readcsk2* .
  cp ../readpart* .
  cp ../readmthi* .
  cp ../suml* .
  ...
  cp ../totaljobfile* .
  cp ../totaltimeik3* .
  cp ../postprocess-ik3.sh .
  cp ../sumrawcoreas* .
  cd ..
# - - - - - - end of working in corsika subdirectory.
  echo "           ... cd $cskdirect ... and start the simulation by:"
  tail -1 $jobscript
