...
c  to one new file for each antenna.
c-----------------------------------------------------------------------
c CompLink:
c     gfortran -O0 -fbounds-check sumrawcoreas.f -o sumrawcoreas
c if necessary:
c     ifort -C -O0 -check bounds sumrawcoreas.f -o sumrawcoreas
c Runprog:
...
c  /cr/auger02/joe/corsika.trunk/run/csk072072
c  SIM072072-893463520-000000201_coreas.bins
c-----------------------------------------------------------------------
c  structure numbers in `raw*.dat` files:
...
c-----------------------------------------------------------------------
      ...
      parameter (nrawfilmax=150000)
      character crawname(nrawfilmax)*100, chrawline*100
      character cnewname(10000)*100, charpath*100, crawbins*100
      character chfilerawlst*17, tabkey*1
      dimension qdatraw(0:3,nrawfilmax,400), lrawname(nrawfilmax)
      logical lexist
...
c - - - - get steering quantities from file `simcoreas.rawinfo`:
     ...
     +     form='formatted',access='sequential',status='old')
      read(1,*,end=89) nrunnr
      read(1,*,end=89) nrawtotal
      read(1,*,end=89) nrawsubdir 
      read(1,*,end=89) nrawdat
      read(1,'(a)',end=89) charpath
      read(1,'(a)',end=89) crawbins
      close(unit=1)
      nrawfilmaxldir = nrawtotal / nrawsubdir
      ! - - - - print steering quantities:
      write(chfilerawlst,'(''SIM'',i6.6,''.raw.lst'')') nrunnr
      write(*,'(42('' _''),/,1x,''sumrawcoreas.f'')')
      write(*,'(25x,''run number:'',i7)') nrunnr
      ...
      write(*,'(25x,''nrawfilmaxldir:'',i7)') nrawfilmaxldir
...
c - - - - - - nrawfilmaxldir = nrawtotal / nrawsubdir
...
c - - - - now write back currently summed Antenna data:    
     ...
     +       form='formatted',access='sequential',status='unknown')
        ...
        write(*,*) '   lfil = ',lfil,'   ifil = ',ifil,
     +     '   nrawsubdir = ',nrawsubdir
**      open(unit=8,file=cnewname(nbin),
