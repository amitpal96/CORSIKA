...
[
  ...
  AM_CONDITIONAL([USE_FLUKACERN], test "x$CORLEMODEL" = "xFLUKACERN")
  AM_CONDITIONAL([USE_FLUKAINFN], test "x$CORLEMODEL" = "xFLUKAINFN")
  ...
  case $CORLEMODEL in
      ...
      FLUKACERN  )
                 AC_MSG_RESULT([flukacern])
                 LOMODEL="_flukacern" # for suffix generation for corsika binary/source name
                 CORSIKA_REQUIRE_FLUKACERN
                 CORSIKA_FFLAGS="$CORSIKA_FFLAGS $FLUKA_FFLAGS"
                 MODEL_LIBS="$MODEL_LIBS $FLUKA_LIBS" 
                 AC_DEFINE([__FLUKA__],[1],[include FLUKA program])
                 AC_DEFINE([__FLUKACERN__],[1],[include FLUKA CERN program])
                 CORSIKA_APPEND_CONFLICTS([CONEX])
                 CORSIKA_APPEND_CONFLICTS([DPMJET])
                 CORSIKA_CACHE([FLUKACERN])
                 ;;

      FLUKAINFN  )
                 AC_MSG_RESULT([flukainfn])
                 LOMODEL="_flukainfn" # for suffix generation for corsika binary/source name
                 CORSIKA_REQUIRE_FLUKAINFN
                 CORSIKA_FFLAGS="$CORSIKA_FFLAGS $FLUKA_FFLAGS"
                 MODEL_LIBS="$MODEL_LIBS $FLUKA_LIBS" 
                 AC_DEFINE([__FLUKA__],[1],[include FLUKA program])
                 AC_DEFINE([__FLUKAINFN__],[1],[include FLUKA INFN program])
                 CORSIKA_APPEND_CONFLICTS([DPMJET])
                 CORSIKA_CACHE([FLUKAINFN])
                 ;;
...
AC_DEFUN([CORSIKA_ENABLE_NRREXT],
[_CORSIKA_ENABLE_OPTION([NRREXT], [ANAHIST AUGERHIST INTTEST PLOTSH PLOTSH2], [], [no], [extended RUNNR], [__NRREXT__], [])]) 
...
AC_DEFUN([CORSIKA_ENABLE_HIGHMEM],
[_CORSIKA_ENABLE_OPTION([HIGHMEM], [], [], [no], [to compile EPOS with extended array for heavy mass], [__HIGHMEM__], [])])
...
#    CONEX_FFLAGS="-O0 -I\$(srcdir)"
    ...
    case x$CORLEMODEL in
       ...
       xFLUKAINFN ) CONEX_CPPFLAGS="$CONEX_CPPFLAGS -D__FLUKA__"
...
#      FLUKA_LIB
#
AC_DEFUN([CORSIKA_WITH_FLUKACERN],
[_CORSIKA_WITH_PACKAGE([flukacern],
  [AC_HELP_STRING([--with-flukacern],
                  [use fluka-cern (default at FLUPRO)])],
  [$FLUPRO],
  ["include/flkcmp.inc"])
if test x$corsika_cv_root_flukacern != xno ; then
   FLUPRO=$corsika_cv_root_flukacern
   FLUKA_CONFIG=$FLUPRO/bin/fluka-config
   FLUKA_FFLAGS="$($FLUKA_CONFIG --flags) $($FLUKA_CONFIG --include)"
   FLUKA_LIBS="$($FLUKA_CONFIG --libpath) $($FLUKA_CONFIG --lib)"
   _CORSIKA_SAVE_COMPILE_FLAGS
   FFLAGS="$FFLAGS $FLUKA_FFLAGS"
   LIBS="$LIBS $FLUKA_LIBS"
   AC_MSG_CHECKING(whether FLUKA-CERN can be used)
   AC_LANG_PUSH([Fortran 77])
   AC_LINK_IFELSE(
      [AC_LANG_PROGRAM([],[[
                            include 'iounit.inc'
                        ...
                        [
                          AC_MSG_RESULT(yes)
                          AC_DEFINE([__LINUX__],[1],[FLUKA for Linux])
                        ],
                        [
                          AC_MSG_RESULT(no)
                          corsika_cv_root_flukacern="no"
                        ]) 
   _CORSIKA_RESTORE_COMPILE_FLAGS
   AC_LANG_POP([Fortran 77])
   ...
   AC_SUBST([FLUKA_LIBS])
   AC_SUBST([FLUPRO])
   AC_DEFINE([HAVE_FLUKACERN], 1)
   AH_TEMPLATE([HAVE_FLUKACERN], [Define to 1 if FLUKA-CERN is available on the system])
fi
])


# CORSIKA_REQUIRE_FLUKACERN
#   Require that FLUKA-CERN is installed, if not, configuration fails
#
AC_DEFUN([CORSIKA_REQUIRE_FLUKACERN],
[AC_REQUIRE([CORSIKA_WITH_FLUKACERN])
if test x$corsika_cv_root_flukacern = xno ; then
  CORSIKA_MISSING_REQUIRED([FLUKACERN])
fi
])
...
#      FLUKA_LIB
#
AC_DEFUN([CORSIKA_WITH_FLUKAINFN],
[_CORSIKA_WITH_PACKAGE([flukainfn],
  [AC_HELP_STRING([--with-flukainfn],
                  [use fluka-infn (default at FLUPRO)])],
  [$FLUPRO],
  ["flukapro/(FLKCMP)"])
  corsika_fluka_need_aamodvvax="no"
if test x$corsika_cv_root_flukainfn != xno ; then
   FLUPRO=$corsika_cv_root_flukainfn
   FLUKA_FFLAGS="$FLUKA_FFLAGS -I$FLUPRO/flukapro"
   FLUKA_LIBS="$FLUKA_LIBS -L$FLUPRO -lflukahp"
   _CORSIKA_SAVE_COMPILE_FLAGS
   FFLAGS="$FFLAGS $FLUKA_FFLAGS"
   LIBS="$LIBS $FLUKA_LIBS"
   AC_MSG_CHECKING(whether FLUKA-INFN can be used)
   AC_LANG_PUSH([Fortran 77])
   AC_LINK_IFELSE(
      [AC_LANG_PROGRAM([],[[
                            include '(IOUNIT)'
                        ...
                        [
                          AC_MSG_RESULT(no)
                          corsika_cv_root_flukainfn="no"
                        ]) 
   _CORSIKA_RESTORE_COMPILE_FLAGS
   AC_LANG_POP([Fortran 77])
   AC_CHECK_FILE($FLUPRO/aamodmvax, [corsika_fluka_need_aamodvvax="yes"])
   AC_SUBST([FLUKA_FFLAGS])
   AC_SUBST([FLUKA_LIBS])
   AC_SUBST([FLUPRO])
   AC_DEFINE([HAVE_FLUKAINFN], 1)
   AH_TEMPLATE([HAVE_FLUKAINFN], [Define to 1 if FLUKA-INFN is available on the system])
fi
AM_CONDITIONAL([FLUKA_NEED_AAMODVVAX], test "x$corsika_fluka_need_aamodvvax" = "xyes")
])


# CORSIKA_REQUIRE_FLUKAINFN
#   Require that FLUKA-INFN is installed, if not, configuration fails
#
AC_DEFUN([CORSIKA_REQUIRE_FLUKAINFN],
[AC_REQUIRE([CORSIKA_WITH_FLUKAINFN])
if test x$corsika_cv_root_flukainfn = xno ; then
  CORSIKA_MISSING_REQUIRED([FLUKAINFN])
fi
])
